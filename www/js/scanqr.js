...
export async function requestQRAndDisplay(targetURLRead, displayQRPage, callerType) {
  // Flag to perform verification of received credential
  let verifyCred = false;
...
// Start the camera to scan the QR
// The scan can be used either by the Passenger or the Verifier
export async function initiateReceiveQRScanning2(
  _canvasElement,
  _qrMessageElement,
  _displayQRPage,
  _callerType
...
//      setTimeout(ReceiveQRtick, scanRefreshInterval);
    ...
    // Get a video frame and decode an image data using the canvas element
    canvas.drawImage(video, 0, 0, elwidth, elwidth);
    var imageData = canvas.getImageData(
      0,
      0,
      canvasElement.width,
