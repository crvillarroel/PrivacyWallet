...
async function requestQRAndDisplay(targetURLRead, displayQRPage, callerType) {
    ...
    try {
        ...
        console.log("Received credential from Issuer", data);
    } catch (error) {
...
pages["#credentialListPage"] = async function (pageData) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (credentialType == "hcert") {
            ...
            html += HCERT.renderSummary(key, credential)
            continue;
...
document.querySelector("#displayQRButton").onclick = async function displayQRButton() {
    let pageData = window.history.state.pageData
    gotoPage("#displayQRPage", { pageData })
}


document.querySelector("#saveCredentialButton").onclick = async function saveCredentialButton() {
...
document.querySelector("#deleteButton").onclick = async function deleteButton() {
    $('#passengerDeleteCredModal').modal("show")
    return;
}

document.querySelector("#deleteCredentialButton").onclick = async function deleteCredential() {
...
async function tuboStartup() {
    ...
    tuboVideoEl.oncanplay = (event) => {
        ...
        codeReader.decodeOnceFromStream(globalStream).then((result) => {
            console.log(result)
            stopAllMediaTracks()
...
document.querySelector("#adminClearLogsButton").onclick = function adminClearLogsButton() {
    clearLogs()
}

document.querySelector("#adminResetDatabaseButton").onclick = function adminResetDatabaseButton() {
    resetDatabase()
}
