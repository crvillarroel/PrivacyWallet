...
export var trustedList = {
    ...
    init: async function() {
        if (this.publicKeys === undefined) {
            try {
                let response = await fetch("assets/all_jwk_keys.json")
    ...
    get: async function (kid) {
        ...
        if (!kid) { throw new Error("kid is undefined") }

        let entry = this.publicKeys[kid]
        if (entry === undefined) { throw new Error(`Entry not found for kid: ${kid}`) }

        let jwk = entry["jwk"]
        if (jwk === undefined) { throw new Error(`Entry for kid: ${kid} des not have jwk`) }
...
export var vs = {
    ...
    init: async function (force = false) {
        try {
            this.valueSets = await $.get("assets/value-sets.json")
...
export class CWT {
    ...
    static async decodeCWT(data, verify) {
        ...
        if (verify) {
            ...
            let keyJWK = undefined
            try {
                keyJWK = await trustedList.get(kid)                
            } catch (error) {
                console.error(`Key ${kid} is NOT in PRODUCTION LIST`)
                if (PREKeys.includes(kid)) {
                    verified = "PRE"
                    console.warn(`KEY ${kid} found in PRE LIST`)
                }
            }
